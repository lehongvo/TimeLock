...
/* is IERC165  */ interface IERC721Base {
...
/* is IERC721Base */ interface IERC721Enumerable {
...
interface AxieSpawningManager {
    function isSpawningAllowed(
        uint256 _genes,
        address _owner
    ) external returns (bool);

    function isRebirthAllowed(
        uint256 _axieId,
        uint256 _genes
    ) external returns (bool);
}
...
contract AxieDependency {
    ...
    function setGeneScientist(
        address _geneScientist,
        bool _whitelisted
    ) external onlyWhitelistSetter {
...
contract AxieERC721BaseEnumerable is
    ERC165,
    IERC721Base,
    IERC721Enumerable,
    AxieDependency,
    AxiePausable
{
    ...
    function _isTokenOperator(
        address _operatorToCheck,
        uint256 _tokenId
    ) private view returns (bool) {
        return
            whitelistedMarketplace[_operatorToCheck] ||
            _tokenOperator[_tokenOwner[_tokenId]][_operatorToCheck];
    }

    function _isApproved(
        address _approvedToCheck,
        uint256 _tokenId
    ) private view returns (bool) {
    ...
    function ownerOf(
        uint256 _tokenId
    ) external view mustBeValidToken(_tokenId) returns (address) {
        ...
        whenTransferAllowed(_from, _to, _tokenId)
    {
        ...
        if (_check && _isContract(_to)) {
            IERC721TokenReceiver(_to).onERC721Received.gas(50000)(
                _from,
                _tokenId,
                _data
            );
...
/* is IERC721Base */ interface IERC721Metadata {
...
contract AxieERC721Metadata is AxieERC721BaseEnumerable, IERC721Metadata {
    ...
    function tokenURI(
        uint256 _tokenId
    ) external view mustBeValidToken(_tokenId) returns (string) {
        ...
        do {
            _tokenURIBytes[_i--] = bytes1(48 + (_tmpTokenId % 10));
            _tmpTokenId /= 10;
        } while (_tmpTokenId > 0);
...
contract AxieCore is AxieERC721 {
    ...
    function retireAxie(
        uint256 _axieId,
        bool _rip
    ) external onlyByeSayer whenRetirementAllowed(_axieId, _rip) {
        _burn(_axieId);
